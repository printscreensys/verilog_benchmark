    ...
    always @(posedge tck or negedge trst_n) begin
        if (!trst_n) begin
            ...
            case (state)
                ...
                ST_SHIFT_IR: begin
                    // INTENTIONAL FLAW: shifts and samples the instruction register in the wrong direction.
                    tdo <= ir_shift[3];
                    ir_shift <= {ir_shift[2:0], tdi};
                end
                ST_UPDATE_IR: begin
                    ir <= ir_shift;
                end
                ...
                ST_SHIFT_DR: begin
                    // INTENTIONAL FLAW: shifts and samples the data register in the wrong direction.
                    tdo <= dr_shift[7];
                    dr_shift <= {dr_shift[6:0], tdi};
                end
                ST_UPDATE_DR: begin
