    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!active) begin
                ...
                if (current_match && !found_seen) begin
                    found_seen <= 1'b1;
                    first_index_seen <= byte_index;
                end

                if (byte_index == 3'd7) begin
                    active <= 1'b0;
                    done <= 1'b1;
                    found <= found_seen | current_match;
                    if (found_seen) begin
                        first_index <= first_index_seen;
